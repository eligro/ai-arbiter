...
const SignupPage = () => {
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    try {
      ...
      // For this app, we'll navigate them to a page telling them to check their email.
      // Or, if email confirmation is disabled, onAuthStateChange will log them in.
      // Assuming email confirmation is disabled for simplicity.
      navigate('/dashboard');
    } catch (err: any) {
    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-background flex items-center justify-center p-4">
      ...
      >
        <div className="text-center">
          ...
          <p className="text-text-secondary mt-2">Start resolving disputes today.</p>
        </div>
        ...
        <button
          ...
          className="w-full flex justify-center items-center gap-3 py-3 px-4 bg-white text-gray-900 font-semibold rounded-lg border border-gray-300 hover:bg-gray-50 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:scale-100 shadow-sm"
        >
          {isGoogleLoading ? (
